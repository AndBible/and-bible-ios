...
public final class RemoteSyncBookmarkPlaybackSettingsStore {
    ...
    /**
     Reads the exact canonical fidelity rows used by complete bookmark snapshot projection.

     The public inventory retains its historical parseable-UUID behavior. Snapshot projection uses
     this narrower boundary so it observes only the lowercase key generated by
     `playbackSettingsJSON(for:kind:)`, ignoring case aliases exactly as point lookup does.

     - Returns: Canonical entries sorted by bookmark kind and UUID string.
     - Side effects: Reads only the playback-fidelity settings namespace.
     - Failure modes: Settings fetch failures retain the store's fail-soft empty-result behavior.
     */
    func snapshotEntries() -> [Entry] {
        settingsStore.entries(inExactNamespace: Keys.prefix)
            .compactMap { entry in
                guard let decoded = decodeEntry(entry),
                      entry.key == scopedKey(
                          bookmarkID: decoded.bookmarkID,
                          kind: decoded.bookmarkKind
                      ) else {
                    return nil
                }
                return decoded
