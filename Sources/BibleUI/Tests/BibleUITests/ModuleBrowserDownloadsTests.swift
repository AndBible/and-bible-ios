...
final class ModuleBrowserDownloadsTests: XCTestCase {
    ...
    /**
     ...
     modules selected by the default-document flow; failed defaults stay strict for row retries while
     unrelated manual installs opened later in the same Downloads session keep raw fallback.
     */
    ...
    /**
     Verifies failed Easy Start defaults stay package-only after startup activity finishes.

     The fixture drives the same reducer that `ModuleBrowserView` stores in `@State`: default
     planning inserts KJV/NASB into active and strict sets, a KJV failure removes it from active
     installs but keeps strict retry membership, and finishing activity must not clear that failed
     strict entry. A failure here means the visible retry path can downgrade back to raw fallback and
     recreate the partial-Bible install from issue 354.
     */
    func testModuleBrowserDefaultDownloadStrictPackageStateRetainsFailedModuleForRetry() {
        var state = ModuleBrowserDefaultDownloadStrictPackageState()

        state.startInstalling(Set(["KJV", "NASB"]))

        XCTAssertEqual(state.installingModules, Set(["KJV", "NASB"]))
        XCTAssertEqual(state.strictModules, Set(["KJV", "NASB"]))
        XCTAssertFalse(
            state.finish("KJV", strictPolicyResolution: .retainForRetry),
            "NASB is still active, so the default activity should remain open."
        )
        XCTAssertEqual(state.installingModules, Set(["NASB"]))
        XCTAssertEqual(state.strictModules, Set(["KJV", "NASB"]))
        XCTAssertEqual(
            state.packageInstallPolicy(mode: .englishStartup, moduleName: "KJV"),
            .requirePackage
        )
        XCTAssertEqual(
            state.packageInstallPolicy(mode: .englishStartup, moduleName: "TR"),
            .preferPackageThenRaw
        )

        XCTAssertTrue(state.finish("NASB", strictPolicyResolution: .clear))
        state.finishActivity()

        XCTAssertEqual(state.installingModules, Set<String>())
        XCTAssertEqual(state.strictModules, Set(["KJV"]))
        XCTAssertEqual(
            state.packageInstallPolicy(mode: .englishStartup, moduleName: "KJV"),
            .requirePackage
        )
        XCTAssertEqual(
            state.packageInstallPolicy(mode: .englishStartup, moduleName: "NASB"),
            .preferPackageThenRaw
        )
    }

    /**
     Verifies successful installs and user cancellations clear Easy Start strict retry state.

     `ModuleBrowserView` uses the same `.clear` reducer transition for successful installs and
     explicit row cancellations. The expected result pins the conscious design choice that a module
     whose default request ended cleanly is no longer owned by the startup strict policy; a later
     in-session row tap behaves like a normal manual Downloads install.
     */
    func testModuleBrowserDefaultDownloadStrictPackageStateClearsAfterSuccessOrCancel() {
        var state = ModuleBrowserDefaultDownloadStrictPackageState()

        state.startInstalling(Set(["KJV"]))

        XCTAssertTrue(state.finish("KJV", strictPolicyResolution: .clear))
        state.finishActivity()

        XCTAssertEqual(state.installingModules, Set<String>())
        XCTAssertEqual(state.strictModules, Set<String>())
        XCTAssertEqual(
            state.packageInstallPolicy(mode: .englishStartup, moduleName: "KJV"),
            .preferPackageThenRaw
        )
