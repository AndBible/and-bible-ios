...
/**
 Decodes the HTML 4 entity set used by Android's OSIS XML serializer.

 Android calls Apache Commons Text `StringEscapeUtils.unescapeHtml4` for every ordinary XML text
 node immediately before serialization. This decoder keeps that boundary deterministic without
 invoking an HTML renderer or accepting HTML5-only named entities that Android leaves untouched.
 */
enum SwordHTML4EntityDecoder {
    /**
     Decodes semicolon-terminated named and numeric HTML 4 entities.

     - Parameter text: Parsed XML text that may still contain a second layer of entity encoding.
     - Returns: Text with Android's HTML 4 entities decoded; unknown or malformed entities remain
       byte-for-byte unchanged.
     - Side effects: None.
     - Failure modes: Invalid numeric scalars and unknown names are preserved verbatim.
     - Complexity: Linear in the number of Swift characters plus constant-time entity lookups.
     */
    static func decode(_ text: String) -> String {
        ...
        while cursor < text.endIndex {
            ...
            let bodyStart = text.index(after: cursor)
            let searchEnd = text.index(
                bodyStart,
                offsetBy: 32,
                limitedBy: text.endIndex
            ) ?? text.endIndex
            guard let semicolon = text[bodyStart..<searchEnd].firstIndex(of: ";") else {
                output.append("&")
                cursor = bodyStart
                continue
            }
            ...
            guard let decoded = decodedEntity(body) else {
                output.append("&")
                cursor = bodyStart
                continue
            }
    ...
    /**
     Resolves one entity body without its surrounding ampersand and semicolon.

     - Parameter body: Named, decimal numeric, or hexadecimal numeric entity body.
     - Returns: One decoded Unicode scalar, or `nil` for unsupported/invalid input.
     - Side effects: None.
     - Failure modes: HTML5-only names, missing digits, surrogate values, and out-of-range
       codepoints return `nil`.
     */
    private static func decodedEntity(_ body: String) -> String? {
        ...
        if body.hasPrefix("#x") || body.hasPrefix("#X") {
            ...
            scalarValue = namedScalars[body]
        }
        guard let scalarValue, let scalar = UnicodeScalar(scalarValue) else { return nil }
        return String(scalar)
    }

    /// Apache Commons Text's case-sensitive HTML 4 named-entity table.
    private static let namedScalars: [String: UInt32] = [
        ...
        "spades": 0x2660, "sub": 0x2282, "sube": 0x2286,
        "sum": 0x2211, "sup": 0x2283, "sup1": 0x00B9,
        "sup2": 0x00B2, "sup3": 0x00B3, "supe": 0x2287,
        "szlig": 0x00DF, "tau": 0x03C4, "there4": 0x2234,
        "theta": 0x03B8, "thetasym": 0x03D1, "thinsp": 0x2009,
        "thorn": 0x00FE, "tilde": 0x02DC, "times": 0x00D7,
        "trade": 0x2122, "uArr": 0x21D1, "uacute": 0x00FA,
        "uarr": 0x2191, "ucirc": 0x00FB, "ugrave": 0x00F9,
        "uml": 0x00A8, "upsih": 0x03D2, "upsilon": 0x03C5,
        "uuml": 0x00FC, "weierp": 0x2118, "xi": 0x03BE,
        "yacute": 0x00FD, "yen": 0x00A5, "yuml": 0x00FF,
