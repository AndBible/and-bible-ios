...
extension AndBibleTests {
     ...
     */
    func testWindowPaneMenuNormalWindowTopLevelRowsMatchAndroidOrder() {
        let currentID = UUID()
        let otherID = UUID()
     ...
     */
    func testWindowPaneMenuLinksWindowShowsChangeToNormalAndHidesNormalRows() {
        let currentID = UUID()
        let model = BibleWindowPaneMenuModel(snapshot: .fixture(
            windowID: currentID,
            isLinksWindow: true,
     ...
     */
    func testWindowPaneMenuSynchronizedSubmenuShowsDisableAndSkipsCurrentGroup() throws {
        let model = BibleWindowPaneMenuModel(snapshot: .fixture(
            isSynchronized: true,
            syncGroup: 2
        ))
     ...
     */
    func testWindowPaneMenuMoveToSubmenuUsesAbsolutePositionsInSamePinBucket() throws {
        let firstID = UUID()
        let currentID = UUID()
        let lastID = UUID()
    ...
    /**
     ...
     Android XML marks `pinMode` checkable even when it is unchecked, and dynamic text-display
     preference rows are checkable booleans. The custom iOS renderer needs that metadata so it can
     draw an empty checkbox instead of treating `false` as a plain command row.
     */
    func testWindowPaneMenuCheckableRowsExposeUncheckedStateForRenderer() throws {
        let model = BibleWindowPaneMenuModel(snapshot: .fixture(
            isPinned: false,
     ...
     */
    func testWindowPaneMenuCopyLinkURLUsesAndroidReadAndBibleSemantics() throws {
