...
class UITestFixtureService:
    ...
    def _data_container(self, deadline: float) -> Path:
        """Resolve the installed app container, bootstrapping it before any mutation if absent."""
        result = self._lookup_data_container(deadline)
        container = self._validated_data_container(result)
        if container is not None:
            return container

        self._require_success(
            "simulator app bootstrap launch",
            [
                "/usr/bin/xcrun",
                "simctl",
                "launch",
        ...
        if container is None:
            if result.returncode != 0:
                raise FixtureServiceError(
                    self._command_failure("installed app data-container lookup", result)
                )
            raise FixtureServiceError(
                "installed app data-container lookup did not return one available absolute path"
            )
        return container

    def _lookup_data_container(self, deadline: float) -> CommandResult:
        """Run one bounded host lookup without treating a missing first-launch container as fatal."""
        return self._run(
            [
                "/usr/bin/xcrun",
                "simctl",
    ...
    @staticmethod
    def _validated_data_container(result: CommandResult) -> Path | None:
        """Return a real absolute container directory from a successful lookup."""
        if result.returncode != 0:
            return None
