...
/**
 * Navigate to an internal link (ab-w://, osis://, multi://).
 * On iOS, window.location.assign() is unreliable for custom schemes in WKWebView,
 * so we route through the bridge's postMessage directly.
 * On Android, location.assign() triggers shouldOverrideUrlLoading reliably.
 */
export function navigateLink(url: string) {
    if ((window as any).__PLATFORM__ === 'ios' && (window as any).webkit?.messageHandlers?.bibleView) {
        (window as any).webkit.messageHandlers.bibleView.postMessage({
