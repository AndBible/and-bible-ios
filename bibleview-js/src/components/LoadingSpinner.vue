...
/**
 * Renders the Android shared-reader loading indicator for document and manual chapter loads.
 *
 * @remarks
 * The component reads `appSettings.disableAnimations` from the injected reader config so the
 * indicator respects the same animation preference as Android. It has no persistence side effects
 * and fails only if the shared app-settings provider is missing, which indicates BibleView was
 * mounted outside its normal provider tree.
 */
