...
| Persistence model | Android persists `CustomRepository` rows with name, description, type, host, catalog directory, package directory, and manifest URL. | Adapted Pass | iOS now keeps SWORD refresh rows in `InstallMgr.conf` and mirrors Android custom metadata in `CustomRepositories.json`, including description, type, package directory, manifest URL, and source URL for edit/delete round-tripping. |
