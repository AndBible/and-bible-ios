...
| Delete / uninstall | Installed rows expose `delete` when Android `installedDoc.canDelete`; confirmation asks before deleting and then reloads documents. | Adapted Pass | iOS installed catalog rows expose Uninstall through swipe and context menu, now behind confirmation before removing local SWORD module files and refreshing installed state. If an update/install task is active for that module, iOS cancels and awaits the installer rollback path before deletion so staged files cannot race removal. iOS does not currently model a separate `canDelete` flag, so installed rows are treated as removable. |
