...
| Sync settings UI persists backend and category state across reopen | `SyncSettingsView.swift`; package tests `testApplicationPreferenceFeatureShortcutsExposeAndroidRows`, `testApplicationPreferenceFeatureShortcutsFollowRuntimeAvailability`; UI tests `testSyncSettingsCategoryDisablePersistsAcrossDirectReopen`, `testSyncSettingsMyDocumentsCategoryToggleStartsManualSyncPath`, `testSyncSettingsBackendSwitchPersistsAcrossDirectReopen` | Pass | Package coverage protects the Settings feature shortcut row contract; the remaining UI gates cover immediate mutation, persisted state, and visible section changes between NextCloud/WebDAV and iCloud. |
