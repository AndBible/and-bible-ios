...
`../bridge/my-documents-model.md`; #104 records the Android sync schema and
policy in `mydocuments-schema.md`, while runtime sync remains split across
#105, #106, #108, #107, and #109. The `ai_settings` category is blocked on the
shared AI backend direction in #5 and the #53/#89 bridge disposition. The
`progress` category has the local iOS reading-progress model/storage/settings
contract recorded in `../bridge/reading-progress-model.md`, but remote sync
remains blocked on #73 and the #52 bridge disposition.
