...
- Reset/status visibility still needs stricter behavioral parity.
- #73 and #74 own the Android `progress` and `ai_settings` sync implementations.
...
| Android category breadth and visible toggle behavior | Android `SyncableDatabaseDefinition` includes `BOOKMARKS`, `WORKSPACES`, `READINGPLANS`, `MYDOCUMENTS`, `AI_SETTINGS`, and `PROGRESS`; Android `SyncSettings.kt` hides `sync_enable_readingplans`; iOS `SyncSettingsPresentation.visibleCategoryRows`; package tests `testSyncSettingsVisibleCategoryRowsMatchAndroidRuntimeVisibility`, `testDeferredSyncCategoriesReserveAndroidCompatibleKeysAndTrackingIssues` | Adapted Pass | iOS hides Reading Plans from settings, keeps its sync engine intact, and shows AI Settings plus Reading Progress as disabled deferred rows until #74/#73 implement those sync streams. |
| Sync settings row presentation, grouping, dynamic visibility, and status behavior | `SyncSettingsView.swift`, `SyncSettingsPresentation.swift`, `AndBibleIconCatalog.swift`; tests `testSyncSettingsPresentationUsesAndroidBackedRows`, `testSyncCategoryPresentationMatchesAndroidCategoryIcons`, `testSyncSettingsVisibleCategoryRowsMatchAndroidRuntimeVisibility`, and Sync Settings UI tests | Partial | Icons, category row order, and visible/deferred category state are source-backed. Reset/status visibility still needs stricter behavioral parity after backend cleanup. |
| Sync settings UI persists backend and category state across reopen | `SyncSettingsView.swift`; package tests `testApplicationPreferenceFeatureShortcutsExposeAndroidRows`, `testApplicationPreferenceFeatureShortcutsFollowRuntimeAvailability`; UI test `testSyncSettingsCategoryDisableAndBackendSwitchPersistAcrossDirectReopen` | Pass | Package coverage protects the Settings feature shortcut row contract; the retained UI gate covers immediate category mutation, persisted category state, backend switching, and visible section changes between NextCloud/WebDAV and iCloud across direct reopen. |
...
| Adopt-versus-create confirmation branch | `SyncSettingsView.swift`; UI test `testSyncSettingsMyDocumentsAdoptCreateChoiceSynchronizesFromVisibleWorkflow` | Pass | The focused workflow drives the visible My Documents adopt/create sheet, chooses the create-new cloud replacement branch, confirms the destructive reset-cloud alert, and waits for the selected category to finish enabled. |
