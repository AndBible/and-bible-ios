...
| Sync settings UI persists backend and category state across reopen | `SyncSettingsView.swift`; package tests `testApplicationPreferenceFeatureShortcutsExposeAndroidRows`, `testApplicationPreferenceFeatureShortcutsFollowRuntimeAvailability`; UI tests `testSyncSettingsCategoryToggleMutatesExportedState`, `testSyncSettingsCategoryDisablePersistsAcrossDirectReopen`, `testSyncSettingsMyDocumentsCategoryToggleStartsManualSyncPath`, `testSyncSettingsBackendSwitchMutatesVisibleSection`, `testSyncSettingsBackendSwitchPersistsAcrossDirectReopen` | Pass | Package coverage protects the Settings feature shortcut row contract; the remaining UI gates cover persisted state and visible section changes between NextCloud/WebDAV and iCloud. |
