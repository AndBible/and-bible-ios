...
| Android category breadth and visible toggle behavior | Android `SyncableDatabaseDefinition` includes `BOOKMARKS`, `WORKSPACES`, `READINGPLANS`, `MYDOCUMENTS`, `AI_SETTINGS`, and `PROGRESS`; Android `SyncSettings.kt` hides `sync_enable_readingplans`; iOS `SyncSettingsPresentation.visibleCategoryRows`; tests `testSyncSettingsVisibleCategoryRowsMatchAndroidRuntimeVisibility`, `testDeferredSyncCategoriesReserveAndroidCompatibleKeysAndTrackingIssues`, `testSyncSettingsCategoryRowsMatchAndroidRuntimeVisibility` | Adapted Pass | iOS hides Reading Plans from settings, keeps its sync engine intact, and shows AI Settings plus Reading Progress as disabled deferred rows until #74/#73 implement those sync streams. |
