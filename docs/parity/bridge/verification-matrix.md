...
| Strong's content depends on a dedicated `contentType: \"strongs\"` document route | `StrongsSheetView.swift` dedicated `BibleBridge`, `BibleReaderController.buildStrongsMultiDocJSON()`, `DocumentBroker.vue`, and `StrongsDocument.vue`; reader routing gap tracked by #8 | Partial | The bridge payload requirement is real: losing `contentType: \"strongs\"` silently falls back to generic multi-document rendering. The remaining native sheet routing gap is owned by the reader parity workstream. |
