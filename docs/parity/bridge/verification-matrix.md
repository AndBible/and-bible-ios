...
| Full current Android bridge surface breadth | Local comparison of iOS `bibleview-js/src/composables/android.ts` with the Android reference checkout's `app/bibleview-js/src/composables/android.ts`; `android-bridge-gap-inventory.json` | Partial | iOS currently exposes 70 bridge methods in its bundled frontend type, while Android exposes 89. The inventory tracks 19 Android-only methods plus 7 resolved iOS bridge dispositions: paragraph-break bookmarks, memorization bridge state, `speakMemorizationLoop`, and My Documents read/copy/share are implemented; My Documents edit/reload and AI page operations remain deferred by #51 into #82 and #83, reading progress is deferred by #52 into #85, #86, and #87, scoped help needs triage, and AI bridge methods are deferred by #53 into #89, #90, #91, and #92. |
