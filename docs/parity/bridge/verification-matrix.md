...
| Full current Android bridge surface breadth | Local comparison of iOS `bibleview-js/src/composables/android.ts` with the Android reference checkout's `app/bibleview-js/src/composables/android.ts`; `android-bridge-gap-inventory.json` | Partial | iOS currently exposes 72 bridge methods in its bundled frontend type, while Android exposes 89. The inventory tracks 17 Android-only methods plus 9 resolved iOS bridge dispositions: paragraph-break bookmarks, memorization bridge state, `speakMemorizationLoop`, My Documents read/copy/share, and My Documents edit/reload are implemented; My Documents AI page operations remain deferred by #51 into #83, reading progress now has the #85 model/settings contract in `reading-progress-model.md` while bridge behavior remains deferred to #86/#87, scoped help needs triage, and AI bridge methods are deferred by #53 into #89, #90, #91, and #92. |
