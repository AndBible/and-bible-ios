...
| Full current Android bridge surface breadth | Local comparison of iOS `bibleview-js/src/composables/android.ts` with the Android reference checkout's `app/bibleview-js/src/composables/android.ts`; `android-bridge-gap-inventory.json` | Partial | iOS currently exposes 66 bridge methods in its bundled frontend type, while Android exposes 89. The inventory tracks 23 Android-only methods plus 3 former no-op dispositions: paragraph-break bookmarks and memorization bridge state are implemented, `speakMemorizationLoop` is deferred by #50 into #78, My Documents is deferred by #51 into #80, #81, #82, and #83, reading progress is deferred by #52 into #85, #86, and #87, scoped help needs triage, and AI bridge methods are deferred by #53 into #89, #90, #91, and #92. |
