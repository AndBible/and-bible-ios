...
| Bookmark, label, and StudyPad delegate dispatch remains centralized in `BibleBridge` | `BibleBridge.dispatchMessage(method:args:)` bookmark and StudyPad switch branches; `BridgeTypes.swift` payload models; unit test `testBridgeMessageDispatchClassifiesKnownMalformedMessages` | Partial | Known malformed required and wrong-type positional arguments are now regression-gated, but valid delegate-callback assertions do not yet cover every branch. |
