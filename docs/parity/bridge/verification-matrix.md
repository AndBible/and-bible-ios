...
|---|---|---|---|
| StudyPad handoff stays connected to native persistence and document reload | `BibleBridge.swift`, `BibleReaderController.swift`; UI test `testBookmarkSelectionNavigatesReaderToSeededReference` | Pass | The current focused UI evidence covers the bookmark-label handoff into the embedded StudyPad document path. |
| My Notes visible document lifecycle and note mutation remain connected to native persistence | `BibleBridge.swift`, `BibleReaderController.swift`; UI test `testMyNotesPseudoDocumentOpensFromChooser`; package tests `testBookmarkServiceClearingBibleBookmarkNoteDeletesPersistedNoteRow`, `testBookmarkServiceClearingBibleBookmarkNoteRemovesBookmarkFromMyNotesQuery`, `testBookmarkServiceUpdatingBibleBookmarkNoteReusesPersistedNoteRow`, `testReaderSaveBookmarkNoteUsesConfiguredMarkdownContentTypeForNewNote`, `testReaderSaveBookmarkNoteDeletesWhitespaceOnlyNotesLikeAndroidBridge` | Pass | The production reader path still opens the Android-style My Notes pseudo-document; bridge and service tests lock note mutation, native persistence, document rebuild data, and delete semantics below the UI. |
...
| Full current Android bridge surface breadth | Local comparison of iOS `bibleview-js/src/composables/android.ts` with the Android reference checkout's `app/bibleview-js/src/composables/android.ts`; `android-bridge-gap-inventory.json` | Partial | iOS currently exposes 81 bridge methods in its bundled frontend type, while Android exposes 89. The inventory tracks 10 Android-only methods plus 16 resolved iOS bridge dispositions: paragraph-break bookmarks, memorization bridge state, `speakMemorizationLoop`, My Documents read/copy/share/edit/reload/AI page actions, and reading-progress mutation/history/UI/settings behavior are implemented; scoped help needs triage, and AI bridge methods are deferred by #53 into #89, #90, #91, and #92. |
...
| Strong's content depends on a dedicated `contentType: \"strongs\"` document route | `BibleReaderController.buildStrongsMultiDocJSON()`, `BibleReaderController.loadDefinitionDocument(...)`, `DocumentBroker.vue`, and `StrongsDocument.vue`; unit tests `testStrongsLinkEmitsVueDocumentInsteadOfNativeSheet` and `testStrongsLinkUsesLinksWindowRoutingCallbackWhenAvailable` | Pass | Strong's and dictionary payloads preserve `contentType: \"strongs\"` and now route through the shared reader document/window pipeline instead of the removed native sheet. |
| Fullscreen, compare, help, external-link, and reference-dialog entry points remain exposed through the bridge | `BibleBridge.swift` switch branches for `toggleFullScreen`, `compare`, `helpDialog`, `openExternalLink`, and `refChooserDialog`; `BibleReaderController.swift` handlers | Partial | These branches are real and still parity-relevant, but they are not yet backed by focused bridge-domain regression coverage. Compare routing specifically is tracked by #123 in the reader workstream. |
