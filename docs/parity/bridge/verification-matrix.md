...
| Full current Android bridge surface breadth | Local comparison of iOS `bibleview-js/src/composables/android.ts` with the Android reference checkout's `app/bibleview-js/src/composables/android.ts`; `android-bridge-gap-inventory.json` | Partial | iOS currently exposes 67 bridge methods in its bundled frontend type, while Android exposes 89. The inventory tracks 22 Android-only methods plus 4 resolved iOS bridge dispositions: paragraph-break bookmarks, memorization bridge state, and `speakMemorizationLoop` are implemented, My Documents is deferred by #51 into #80, #81, #82, and #83, reading progress is deferred by #52 into #85, #86, and #87, scoped help needs triage, and AI bridge methods are deferred by #53 into #89, #90, #91, and #92. |
