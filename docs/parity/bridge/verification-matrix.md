...
| Full current Android bridge surface breadth | Local comparison of iOS `bibleview-js/src/composables/android.ts` with the Android reference checkout's `app/bibleview-js/src/composables/android.ts`; `android-bridge-gap-inventory.json` | Partial | iOS currently exposes 77 bridge methods in its bundled frontend type, while Android exposes 89. The inventory tracks 14 Android-only methods plus 12 resolved iOS bridge dispositions: paragraph-break bookmarks, memorization bridge state, `speakMemorizationLoop`, My Documents read/copy/share/edit/reload/AI page actions, and reading-progress `recordChapterRead` are implemented; reading-progress history/UI/settings remains deferred to #87, scoped help needs triage, and AI bridge methods are deferred by #53 into #89, #90, #91, and #92. |
