...
| [downloads](downloads/README.md) | Downloads list parity is now source-backed and repository source management is aligned for SWORD HTTPS custom sources; startup defaults, progress/cancel/error, row action breadth, MyBible custom repositories, and custom metadata persistence remain gaps | Focused unit coverage for metadata matching, update/status sorting, targeted search seed behavior, and repository source persistence/validation | Default-document startup flow #133, progress/cancel/error state #134, row action breadth #135, add-ons #136, MyBible custom repositories #138, and custom repository metadata #139 |
