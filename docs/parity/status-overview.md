...
| [sync](sync/README.md) | Strong for supported Android categories: `7 Pass`, `2 Adapted Pass`, `1 Partial` | Focused unit/integration coverage, focused Sync UI coverage, explicit guardrails | Android-only category implementation breadth is split into deferred parity targets: `mydocuments` (#72, model contract recorded in bridge docs but implementation pending), `ai_settings` (#74, blocked on #5/#89), and `progress` (#73, blocked on #85) |
