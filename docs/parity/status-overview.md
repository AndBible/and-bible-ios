...
| [reader](reader/README.md) | Reader shell/menu parity is stronger, but the document/modal routing audit still leaves several native reader surfaces as partial: `8 Pass`, `1 Adapted Pass`, `3 Partial` | Focused reader-shell UI coverage, restored-position/config-payload/gesture-policy/modal-state/compare-payload unit regressions, and full local UI validation | Strong's document routing (#8), Compare document routing (#123), and multi-reference document routing (#124) |
