...
| [sync](sync/README.md) | Source re-audited: `8 Pass`, `1 Adapted Pass`, `2 Partial` | Focused unit/integration coverage, focused Sync UI coverage, explicit guardrails | Visible category toggle alignment by #158, reset/status presentation cleanup, and Android-only category implementation breadth by `ai_settings` (#74, blocked on #5/#89) and `progress` (#73, local model/UI bridge in place; remote sync pending) |
