...
| [sync](sync/README.md) | Source re-audited: `8 Pass`, `2 Adapted Pass`, `1 Partial` | Focused unit/integration coverage, focused Sync UI coverage, explicit guardrails | Reset/status presentation cleanup, plus Android-only category implementation breadth by `ai_settings` (#74, blocked on #5/#89) and `progress` (#73, local model/UI bridge in place; remote sync pending) |
...
| [reader](reader/README.md) | Reader shell/menu parity is stronger, Strong's, Compare, and multi-reference links now use the shared document pipeline, and reader presentation routes are classified under ADR 0006: `12 Pass`, `1 Adapted Pass`, `1 Partial` | Focused reader-shell UI coverage, restored-position/config-payload/gesture-policy/modal-state/compare-payload/Strong's-routing/multi-reference-routing unit regressions, reader modal ownership matrix guardrail, and full local UI validation | Document chooser completion (#245) and bookmarks/labels/StudyPad presentation details (#246) |
| [downloads](downloads/README.md) | Downloads list parity is now source-backed, repository source management is aligned for SWORD HTTPS custom sources and Android MyBible custom repository specs, English startup defaults consume Android metadata, active downloads expose progress/cancel/error state, and add-ons are modeled as dedicated `And Bible` rows; full reader-surface integration for installed non-SWORD MyBible packages remains outside the source-management matrix | Focused unit coverage for metadata matching, default-document selection/skipping, update/status sorting, targeted search seed behavior, active download progress/cancel/error state, add-on filtering/metadata behavior, repository source persistence/validation, and MyBible manifest/install handling | Full non-SWORD MyBible reader-surface integration after repository-backed install |
