...
| [downloads](downloads/README.md) | Downloads list parity is now source-backed, repository source management is aligned for SWORD HTTPS custom sources, English startup defaults consume Android metadata, and active downloads expose progress/cancel/error state; row action breadth, add-ons, MyBible custom repositories, and custom metadata persistence remain gaps | Focused unit coverage for metadata matching, default-document selection/skipping, update/status sorting, targeted search seed behavior, active download progress/cancel/error state, and repository source persistence/validation | Row action breadth #135, add-ons #136, MyBible custom repositories #138, and custom repository metadata #139 |
