...
| Workspace creation and switching remain coordinated by the reader shell and workspace model | `BibleReaderView.swift`, `WorkspaceSelectorView.swift`, `WindowManager`, `WorkspaceStore`; UI test `testWorkspaceSelectorCreateFlowReturnsToReaderShell`; package tests `WorkspaceWindowStoreTests` | Pass | The retained UI smoke covers the visible selector-owned create prompt and return to the reader shell. Package coverage owns workspace graph creation, ordering, activation prerequisites, clone/delete behavior, and durable switching state so this no longer needs a second UI reopen/switch cycle. |
