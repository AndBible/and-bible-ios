...
| History jump-back plus destructive clear/delete flows persist through reopen | `HistoryView.swift`, `BibleReaderView.swift`, `BibleReaderController.swift`; UI test `testBookmarkSelectionNavigatesReaderToSeededReference`; package tests `HistoryListPresentationTests` | Pass | The retained bookmark/navigation smoke now also protects the live History jump back into the reader, while the more failure-prone destructive persistence paths stay in app-host-free package coverage. |
| Workspace creation and switching remain coordinated by the reader shell and workspace model | `BibleReaderView.swift`, `WorkspaceSelectorView.swift`, `WindowManager`, `WorkspaceStore`; UI test `testDownloadsRepositoryManagerOpensFromOverflow`; package tests `WorkspaceWindowStoreTests` | Pass | The retained Downloads overflow smoke now covers the visible selector-owned create prompt and return to the reader shell before continuing to Downloads. Package coverage owns workspace graph creation, ordering, activation prerequisites, clone/delete behavior, and durable switching state so this no longer needs a second UI reopen/switch cycle. |
...
| Reader config pushes active-window and display state into the embedded client | `BibleReaderController.buildConfigJSON()`, `BibleReaderController.updateConfig()`, `BibleReaderView.updateDisplaySettings(...)`, `BibleReaderPanePresentationTarget`, `BibleReaderWindowDisplaySettingsMutation`; unit tests `testReaderConfigPayloadIncludesDisplaySettingsAndActiveWindowState`, `testReaderConfigPayloadMarksInactiveWindowWithoutActiveIndicator`; package tests `ReaderActivePaneIsolationTests` | Pass | Focused payload-level coverage now locks the config/appSettings key shape, representative display settings, app preference values, workspace label state, active/inactive window indicator behavior, and pane-scoped bookmark/document/Strong's mutations. |
