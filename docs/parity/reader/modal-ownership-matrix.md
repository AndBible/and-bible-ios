...
| --- | --- | --- | --- | --- |
| `ReaderSheet.bookmarks` | `BookmarkListView` in a reader sheet | `Android app-owned` | `Bookmarks` activity from `MenuCommandHandler` | Adapted reader shell route. Deeper label assignment and StudyPad behavior are tracked by #246. |
...
| `ReaderSheet.readingPlans` | `ReadingPlanListView` in a reader sheet | `Android app-owned` | Reading-plan list/selector activities | Adapted reader shell route. Reading-plan list/start/import breadth remains in the reading-plans domain. |
