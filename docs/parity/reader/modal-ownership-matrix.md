...
| `BibleReaderNavigationDrawerAction.myNotes` | Embedded My Notes document loaded into the active reader pane | `Vue/WebView-owned` | Android drawer `myDocumentsButton` launches `MyDocumentsActivity` | Adapted drawer route with UI and source coverage protecting no `ReaderSheet`, no `ReaderModal`, and no pushed reader destination. Broader `MyDocumentsActivity` list-screen parity is outside #257 unless explicitly scoped. |
