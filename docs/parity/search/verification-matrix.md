...
| Multi-translation selection and grouped result totals | `SearchView.swift` translation picker, `MultiResultGroup`, and rerun-on-selection path; `SearchIndexService.searchMultiple(...)`; package test `SearchIndexServiceQueryTests.testIndexedSearchMultipleReturnsPerModuleBucketsForGroupedTotals`; UI test `testSearchMultiTranslationSelectionUpdatesGroupedTotals`; Android reference uses `selectedTranslations` and `getMultiSearchResults(...)` in `SearchResults.kt` | Pass | Package coverage verifies grouped buckets/counts; the focused UI regression remains for picker dismissal, visible abbreviation summary, and grouped-row navigation. |
